...
// Carregar dados e ativos do localStorage ao iniciar a página
...
window.filteredData = filteredData; // Adiciona filteredData globalmente para uso no mapa
...
// Limpa todos os dados SOMENTE ao carregar um novo arquivo
...
// Carrega dados do arquivo Excel e salva no localStorage
...
function applyFiltersAndSort() {
  try {
    ...
    if (currentTab === 'inativos') {
      ...
      window.filteredData = filteredData; // Atualiza filteredData globalmente
      renderList();
      
      // Atualiza mapa se estiver na aba mapa
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Aplicar filtros e renderizar lista se a aba inativos estiver ativa
  if (currentTab === 'inativos') {
    applyFiltersAndSort();
  }
...
function renderAgenda() {
    ...
    schArray.forEach((sch, schIndex) => {
      ...
      if (client && sch.dateTime) {
        agendaItems.push({
          ...
          id,
          schIndex,
          clientData: client // Armazena os dados completos do cliente
        });
      }
    });
  });
  ...
  agendaItems.forEach(item => {
    ...
    const div = document.createElement('div');
    div.style.cursor = 'pointer'; // Adiciona cursor de ponteiro para indicar que é clicável
    div.innerHTML = `
      <span>${dataStr} - ${item.client}</span>
      <strong>${item.tipo || ''}</strong>
    `;
    
    // Adiciona evento de clique para abrir o modal com os detalhes do cliente
    div.addEventListener('click', (e) => {
      // Evita que o clique no botão "Excluir" abra o modal
